...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save'])) {
    ...
    $stmt = $conn->prepare("INSERT INTO peregrin_3 
...
if (isset($_GET['delete'])) {
    ...
    $stmt = $conn->prepare("DELETE FROM peregrin_3 WHERE id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update'])) {
    ...
    $stmt = $conn->prepare("UPDATE peregrin_3 SET 
        room=?, category=?, type=?, code=?, description=?, quantity=?, `condition`=? 
        WHERE id=?");   
    $stmt->bind_param(
        "sssssisi",
...
<head>
<meta charset="UTF-8">
<title>Peregrin 3 FFE Inventory</title>
...
<h2 class="mb-4 text-center">PEREGRIN 3 <span class="text-danger">FFE</span> INVENTORY</h2>
...
<tbody>
<?php
$result = $conn->query("SELECT * FROM peregrin_3 ORDER BY building DESC");
...
<div id="printSummary" style="display:none;">
    <div class="print-header">
        <h2>PEREGRIN 3 INVENTORY SUMMARY</h2>
        <p>Date: <?= date('d M Y') ?></p>
        <hr>
    </div>
...
<select name="room" class="form-select mb-3" required>
    ...
    <optgroup label="1st Floor">
        <?php for($i=103;$i<=108;$i++) echo "<option>Room $i</option>"; ?>
    </optgroup>
    <optgroup label="2nd Floor">
        <?php for($i=201;$i<=205;$i++) echo "<option>Room $i</option>"; ?>
    </optgroup>
    <optgroup label="3rd Floor">
        <?php for($i=301;$i<=308;$i++) echo "<option>Room $i</option>"; ?>
    </optgroup>
    <optgroup label="4th Floor">
        <?php for($i=401;$i<=408;$i++) echo "<option>Room $i</option>"; ?>
    </optgroup>
</select>
...
document.addEventListener('change', function(e) {
    ...
    const typeOptions = {
        ...
        "Blinds": ["Roller Blinds", "Vertical Blinds", "Venetian Blinds"],
    };
