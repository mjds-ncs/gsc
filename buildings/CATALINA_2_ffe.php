...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save'])) {
    ...
    $stmt = $conn->prepare("INSERT INTO catalina_2
...
if (isset($_GET['delete'])) {
    ...
    $stmt = $conn->prepare("DELETE FROM catalina_2 WHERE id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update'])) {
    ...
    $stmt = $conn->prepare("UPDATE catalina_2 SET 
        room=?, category=?, type=?, code=?, description=?, quantity=?, `condition`=? 
        WHERE id=?");   
    $stmt->bind_param(
        "sssssisi",
...
<head>
<meta charset="UTF-8">
<title>Catalina 2 FFE Inventory</title>
...
<h2 class="mb-4 text-center">CATALINA 2 <span class="text-danger">FFE</span> INVENTORY</h2>
...
<tbody>
<?php
$result = $conn->query("SELECT * FROM catalina_2 ORDER BY building DESC");
...
<div id="printSummary" style="display:none;">
    <div class="print-header">
        <h2>CATALINA 2 INVENTORY SUMMARY</h2>
        <p>Date: <?= date('d M Y') ?></p>
        <hr>
    </div>
...
<select name="room" class="form-select mb-3" required>
    ...
    <optgroup label="3rd Floor">
        <?php for($i=301;$i<=306;$i++) echo "<option>Room $i</option>";  ?>
    </optgroup>
