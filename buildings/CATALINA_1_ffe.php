...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save'])) {
    ...
    $stmt = $conn->prepare("INSERT INTO catalina_1
...
if (isset($_GET['delete'])) {
    ...
    $stmt = $conn->prepare("DELETE FROM catalina_1 WHERE id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update'])) {
    ...
    $stmt = $conn->prepare("UPDATE catalina_1 SET 
        room=?, category=?, type=?, code=?, description=?, quantity=?, `condition`=? 
        WHERE id=?");   
    $stmt->bind_param(
        "sssssisi",
...
<head>
<meta charset="UTF-8">
<title>Catalina 1 FFE Inventory</title>
...
<h2 class="mb-4 text-center">CATALINA 1 <span class="text-danger">FFE</span> INVENTORY</h2>
...
<tbody>
<?php
$result = $conn->query("SELECT * FROM catalina_1 ORDER BY building DESC");
...
<div id="printSummary" style="display:none;">
    <div class="print-header">
        <h2>CATALINA 1 INVENTORY SUMMARY</h2>
        <p>Date: <?= date('d M Y') ?></p>
        <hr>
    </div>
...
<select name="room" class="form-select mb-3" required>
    ...
    <optgroup label="1st Floor">
        <?php for($i=101;$i<=106;$i++){if ($i == 102 || $i == 103) continue; echo "<option>Computer Laboratory $i</option>";} ?>
    </optgroup>
    <optgroup label="2nd Floor">
        <?php for($i=201;$i<=205;$i++) echo "<option>Room $i</option>"; ?>
    </optgroup>
    <optgroup label="3rd Floor">
        <?php for($i=302;$i<=305;$i++) echo "<option>Room $i</option>"; ?>
    </optgroup>
    <optgroup label="4th Floor">
        <?php for($i=401;$i<=404;$i++) echo "<option>Room $i</option>"; ?>
    </optgroup>
</select>
